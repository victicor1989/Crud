...
let unitprice = document.getElementById('preciounitario');
let stock = document.getElementById("existencia");
...
btnadd.addEventListener("click", () => {
    ...
    if (reference.value != "" && description.value != "" && unitprice.value != "" && stock.value != "") {
        ...
        if (searchProd == undefined) { // No encuentra la referencia
            ...
            productos.push({ referencia: reference.value, descripcion: description.value, preciounitario: parseInt(unitprice.value), existencia: parseInt(stock.value) })
            message.textContent = "El producto se ha agregado correctamente  ...";
...
btnsearch.addEventListener('click', () => {
    if (reference.value != "") {
        ...
        if (searchProd != undefined) { // Lo encuentra
            ...
            unitprice.value = searchProd.preciounitario;
            stock.value = searchProd.existencia;
...
btnupdate.addEventListener('click', () => {
    ...
    if (referenciaBuscar == reference.value) {
        ...
        productos.splice(indexProdSearch, 1, { referencia: reference.value, descripcion: description.value, preciounitario: parseInt(unitprice.value), existencia: parseInt(stock.value) })
        message.classList.remove("alert-danger");
        message.classList.remove("d-none");
        message.classList.add("alert-success");
...
btndelete.addEventListener('click', () => {
    ...
    if (findProd != undefined) {
        ...
        if (confirm(`Está seguro de eliminar el producto: ${description.value}`)) {
    ...
    if (turnListado) {
        ...
        productos.map(prod => {
            tablaProductos += `<tr>
                                ...
                                <td style='color:${prod.existencia <= 5 ? 'red' : 'green'}'>${prod.existencia <= 5 ? 'Pedir producto' : 'Ok'}</td>
                           <tr>`
        })
